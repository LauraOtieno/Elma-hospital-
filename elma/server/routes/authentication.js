...
router.post('/signup', async (req, res) => {
  ...
  db.query(q,[email],async(err,results)=>{
    ...
    const data = [fullname,email,phone,hashed,usertype];
      const admq="INSERT INTO users (name,email,phone,password,role) VALUES (?,?,?,?,?)";
      db.query(admq,data,(err,result)=>{
        if (err) {
...
router.post('/signin', (req, res) => {
  ...
  console.log(results);
    res.status(200).json({Message:1,Name:ar[0].name,Email:ar[0].emai,Phone:ar[0].phone,Role:ar[0].role});
  });
});
