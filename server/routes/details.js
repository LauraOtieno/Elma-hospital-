...
db.query(query,async(err,resul)=>{
  ...
  else
  {
     if(counter==0)
    {
      res.status(200).json({Message:1,Today:todaypatients,Pending:pendingpayment,Todaypatients:results,Pendingpatients:result,Totalpatients:resul});
    }
    else
    {
